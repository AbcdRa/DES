...
public static class BitArrayExtension
{
    ...
    /*public static void crotl(this BitArray data, int i)
    {
        if (i == 1)
        {
            bool firstBit = data[0];
            data = data.RightShift(1);
            data[data.Length - 1] = firstBit;
            return;
        }
        if (i == 2)
        {
            bool firstBit = data[0];
            bool secondBit = data[1];
            data = data.RightShift(2);
            data[data.Length - 2] = firstBit;
            data[data.Length - 1] = secondBit;
            return;
        }
        throw new Exception("НЕДОПУСТИМЫЙ СДВИГ");
    }*/
}
